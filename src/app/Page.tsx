...
async function loadQboSetupOptions(
  ...
  current = accountResult.connection;
  ...
  const accounts =
    (
      accountResult.data?.QueryResponse
        ?.Account || []
    ) as QboSetupAccount[];
...
>([
  ...
  [
    "expense_account:vendor_bills",
    {
      entityType: "Account",
      purpose: "Vendor Bill Cost / COGS",
    },
  ],
  [
    "expense_account:general_expenses",
    {
      entityType: "Account",
  ...
  [
    ...
    {
      entityType: "Account",
      purpose: "Bank / Checking Payment Account",
    },
  ],
  [
    ...
    {
      entityType: "Account",
      purpose: "Credit Card Payment Account",
    },
  ],
]);
...
function qboPurchasePaymentType(
  paymentMethod: string | null | undefined
) {
  const normalized =
    String(paymentMethod || "")
...
async function requireQboMapping(
  connectionId: string,
  mappingType: string,
  localKey: string,
  purpose: string
...
function qboAccountExpenseLine(
  amount: number,
  description: string,
  account: QboMapping
) {
  return {
    ...
    AccountBasedExpenseLineDetail: {
      AccountRef: {
        value:
          account.qbo_entity_id,
        ...(account.qbo_entity_name
          ? {
              name:
                account.qbo_entity_name,
            }
 ...
 *
 * action=sync_bill
 *   Pushes one Hub Vendor Invoice to QuickBooks as a Bill.
 *   The manufacturer must already be linked to a QBO Vendor.
 *
 * action=sync_bill_payment
 *   Pushes one Hub Vendor Payment to QuickBooks and applies
 *   it to the already-pushed QBO Bill.
 *
 * action=sync_expense
 *   Pushes one Hub Expense to QuickBooks as a Purchase /
 *   expense transaction. All three are create-once, one-way.
 */
  ...
  try {
    ...
    if (action === "sync_bill") {
      ...
      const {
        ...
        .select(
          "id,invoice_number,vendor_invoice_number,manufacturer_id,purchase_order_id,invoice_date,due_date,status,merchandise_amount,freight_amount,tax_amount,other_amount,total_amount,notes,qbo_bill_id,qbo_sync_token,qbo_synced_at"
        )
      ...
      const invoice =
        invoiceData as HubVendorInvoice;
        ...
        return NextResponse.json(
          {
            ...
            error:
              "This Vendor Invoice has already been pushed to QuickBooks.",
            qbo_bill_id:
              invoice.qbo_bill_id,
          },
          {
        ...
        return NextResponse.json(
          {
            status: "not_eligible",
            error:
              "Void Vendor Invoices are not pushed to QuickBooks.",
          },
          {
            status: 409,
          }
        ...
        return NextResponse.json(
          {
            ...
            prerequisite:
              "vendor_sync",
            error:
              "Choose a manufacturer before pushing this Vendor Invoice.",
          },
          {
            status: 409,
          }
        ...
        return NextResponse.json(
          {
            error:
              ...
              "The Vendor Invoice manufacturer could not be loaded.",
          },
          {
            status: 409,
          }
      ...
      if (!manufacturerData.qbo_vendor_id) {
        return NextResponse.json(
          {
            status:
              "prerequisite_required",
            prerequisite:
              "vendor_sync",
            error:
              "Sync this manufacturer to QuickBooks before pushing the Vendor Invoice.",
            manufacturer_id:
              manufacturerData.id,
          },
          {
        ...
        return NextResponse.json(
          {
            status: "not_eligible",
            error:
              "Vendor Invoice total must be greater than zero.",
          },
          {
            status: 409,
          }
      ...
      try {
        const mappingResult =
          await requireQboMapping(
            connection.id,
            "expense_account",
            "vendor_bills",
            "Vendor Bill cost / COGS account"
          );
          ...
          return NextResponse.json(
            {
              status:
                "mapping_required",
              error:
                "Choose the QuickBooks Vendor Bill cost account before pushing this bill.",
              missing_mappings: [
                mappingResult.missing,
              ],
            },
        ...
        const payload:
          Record<string, unknown> = {
            VendorRef: {
              value:
                manufacturerData.qbo_vendor_id,
            },
            TxnDate:
            ...
            DocNumber:
              cleanQboText(
                invoice.vendor_invoice_number ||
                  `DP-BILL-${invoice.invoice_number}`,
                21
              ),
            Line: lines,
            PrivateNote:
              cleanQboText(
                [
                  `Designer’s Patio Hub Vendor Invoice ${invoice.invoice_number}`,
                  invoice.purchase_order_id
                    ? "Linked to a Hub purchase order"
                    : "",
                  invoice.notes || "",
                ]
                  .filter(Boolean)
        ...
        if (!bill?.Id) {
          throw new Error(
            "QuickBooks did not return the created Bill."
          );
        }
        ...
        if (updateError) {
          throw new Error(
            `QuickBooks created the Bill, but the Hub could not save the QuickBooks Bill ID: ${updateError.message}`
          );
        }
        ...
        const message =
          error instanceof Error
            ? error.message
            : "Unknown Vendor Bill sync error";
    ...
    if (action === "sync_bill_payment") {
      const vendorPaymentId =
        String(
          body?.vendor_payment_id || ""
        ).trim();
      ...
      const {
        ...
        .select(
          "id,vendor_invoice_id,manufacturer_id,payment_date,amount,payment_method,reference,notes,qbo_bill_payment_id,qbo_sync_token,qbo_synced_at"
        )
      ...
      const payment =
        paymentData as HubVendorPayment;

      if (
        payment.qbo_bill_payment_id
      ) {
        return NextResponse.json(
          {
            status:
              "already_synced",
            error:
              "This Vendor Payment has already been pushed to QuickBooks.",
            qbo_bill_payment_id:
              payment.qbo_bill_payment_id,
          },
          {
      ...
      const {
        ...
        .from("vendor_invoices")
        .select(
          "id,invoice_number,manufacturer_id,qbo_bill_id"
        )
        .eq(
          "id",
        ...
        return NextResponse.json(
          {
            error:
              invoiceError?.message ||
              "The Vendor Invoice could not be loaded.",
          },
          {
            status: 409,
          }
        ...
        return NextResponse.json(
          {
            ...
            prerequisite:
              "bill_sync",
            error:
              "Push the Vendor Invoice to QuickBooks before pushing its Vendor Payments.",
          },
          {
            status: 409,
          }
      ...
      const manufacturerId =
        invoiceData.manufacturer_id ||
        payment.manufacturer_id;
        ...
        return NextResponse.json(
          {
            ...
            prerequisite:
              "vendor_sync",
            error:
              "This Vendor Payment needs a manufacturer linked to QuickBooks.",
          },
          {
            status: 409,
          }
        ...
        return NextResponse.json(
          {
            error:
              manufacturerError?.message ||
              "The Vendor Payment manufacturer could not be loaded.",
          },
          {
            status: 409,
          }
        ...
        return NextResponse.json(
          {
            ...
            prerequisite:
              "vendor_sync",
            error:
              "Sync this manufacturer to QuickBooks before pushing Vendor Payments.",
          },
          {
            status: 409,
          }
      ...
      try {
        ...
        const paymentAccountResult =
          await requireQboMapping(
            ...
            paymentSourceKey ===
              ...
              : "Bank / Checking payment account"
          );

        if (!paymentAccountResult.mapping) {
          return NextResponse.json(
            {
              status:
                "mapping_required",
              error:
                "Choose the QuickBooks payment account before pushing this Vendor Payment.",
              missing_mappings: [
                paymentAccountResult.missing,
              ],
            },
            {
              status: 409,
        ...
        const payload:
          Record<string, unknown> = {
            ...
            ...(isCreditCard
              ? {
                  CreditCardPayment: {
                    CCAccountRef: {
                      value:
                        paymentAccount.qbo_entity_id,
                      ...(paymentAccount.qbo_entity_name
                        ? {
                            name:
                              paymentAccount.qbo_entity_name,
                          }
            ...
            PrivateNote:
              cleanQboText(
                [
                  `Designer’s Patio Hub payment for Vendor Invoice ${invoiceData.invoice_number}`,
                  payment.payment_method
        ...
        if (!billPayment?.Id) {
          throw new Error(
            "QuickBooks did not return the created Bill Payment."
          );
        }
        ...
        if (updateError) {
          throw new Error(
            `QuickBooks created the Bill Payment, but the Hub could not save its QuickBooks ID: ${updateError.message}`
          );
        }
        ...
        const message =
          error instanceof Error
            ? error.message
            : "Unknown Vendor Payment sync error";
    ...
    if (action === "sync_expense") {
      ...
      const {
        ...
        .from("expenses")
        .select(
          "id,expense_number,expense_date,payee,category,description,amount,payment_method,reference,manufacturer_id,purchase_order_id,sales_order_id,notes,qbo_purchase_id,qbo_sync_token,qbo_synced_at"
        )
        .eq("id", expenseId)
        .single();
        ...
        return NextResponse.json(
          {
            status:
              "already_synced",
            error:
              "This Expense has already been pushed to QuickBooks.",
            qbo_purchase_id:
              expense.qbo_purchase_id,
          },
          {
            status: 409,
          }
      ...
      try {
          ...
          return NextResponse.json(
            {
              status:
                "mapping_required",
              error:
                "Choose the QuickBooks expense and payment accounts before pushing this Expense.",
              missing_mappings:
                missing,
            },
            {
        ...
        const payload:
          Record<string, unknown> = {
            ...
            PrivateNote:
              cleanQboText(
                [
                  `Designer’s Patio Hub Expense ${expense.expense_number}`,
                  expense.payee
                    ? `Payee: ${expense.payee}`
                    : "",
                  expense.payment_method
        ...
        const purchase =
          result.data?.Purchase as
            | QboPurchase
            | undefined;

        if (!purchase?.Id) {
          throw new Error(
            "QuickBooks did not return the created Expense."
          );
        }
        ...
        if (updateError) {
          throw new Error(
            `QuickBooks created the Expense, but the Hub could not save its QuickBooks ID: ${updateError.message}`
          );
        }
